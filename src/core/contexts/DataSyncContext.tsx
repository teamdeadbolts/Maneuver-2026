...
export function DataSyncProvider({ children, maxHistory = 50 }: DataSyncProviderProps) {
  ...
  const updateSync = useCallback((id: string, update: Partial<SyncOperation>) => {
    setOperations(prev => prev.map(op => (op.id === id ? { ...op, ...update } : op)));
  }, []);

  const completeSync = useCallback((id: string, recordsTransferred: number, error?: string) => {
