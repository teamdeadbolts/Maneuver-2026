...
export function WebRTCProvider({ children }: { children: ReactNode }) {
  ...
  const chunksRef = useRef<
    Map<
      string,
      { chunks: string[]; totalChunks: number; scoutName: string; dataType?: TransferDataType }
    >
  >(new Map());
  ...
  const displayName =
    mode === 'lead'
      ...
      : (() => {
          const scoutName =
            typeof window !== 'undefined' ? localStorage.getItem('currentScout') : null;
          const playerStation =
            typeof window !== 'undefined' ? localStorage.getItem('playerStation') : null;
  ...
  const createOfferForScout = useCallback(
    async (scoutName: string): Promise<{ scoutId: string; offer: string }> => {
      ...
      connection.onicecandidate = event => {
        if (event.candidate) {
          ...
          const scout =
            pendingScoutsRef.current.get(scoutId) ||
            connectedScoutsRef.current.find(s => s.id === scoutId);
          if (scout?.signalingPeerId) {
            console.log(
              '🧊 Lead: Sending ICE candidate to scout',
              scout.name,
              'targetPeerId:',
              scout.signalingPeerId
            );
  ...
  const processScoutAnswer = useCallback(
    async (scoutId: string, answerString: string): Promise<void> => {
  ...
  const requestDataFromAll = useCallback((filters?: DataFilters, dataType?: TransferDataType) => {
    console.log(
      `📤 Requesting ${dataType || 'scouting'} data from ${connectedScoutsRef.current.length} scouts...`
    );
    if (filters) {
  ...
  const requestDataFromScout = useCallback(
    (scoutId: string, filters?: DataFilters, dataType?: TransferDataType) => {
  ...
  const pushDataToScout = useCallback(
    (scoutId: string, data: unknown, dataType: TransferDataType) => {
      const scout = connectedScoutsRef.current.find(s => s.id === scoutId);
      if (!scout) {
        console.error(`❌ Scout ${scoutId} not found`);
        return;
  ...
  const startAsScout = useCallback(
    async (scoutName: string, offerString: string): Promise<string> => {
      ...
      connection.onicecandidate = event => {
        if (event.candidate) {
          ...
          console.log(
            '🧊 Scout: Sending ICE candidate to lead, targetPeerId:',
            leadPeerIdRef.current
          );
          signalingRef.current?.sendIceCandidate(
            event.candidate,
            leadPeerIdRef.current || undefined
          );
        } else {
      ...
      await new Promise<void>(resolve => {
        if (connection.iceGatheringState === 'complete') {
          ...
          const timeout = setTimeout(() => {
            console.log(
              `⏱️ Scout ICE gathering timeout (state: ${connection.iceGatheringState}) - proceeding with available candidates`
            );
            connection.onicegatheringstatechange = null;
            resolve();
          }, 5000); // 5 seconds to allow TURN candidates to be gathered
  ...
  const sendData = useCallback((data: unknown, dataType?: TransferDataType) => {
    ...
    if (!dataChannel) {
      const error =
        'ERROR: No data channel exists. Please scan the QR code from the lead scout first.';
      console.error('❌', error);
      alert(error);
      return;
    }
  ...
  const disconnectScout = useCallback(
    (scoutId: string) => {
      ...
      async (message: {
        ...
        try {
          ...
          if (mode === 'lead' && message.type === 'join' && message.role === 'scout') {
            ...
            const existingConnected = connectedScoutsRef.current.find(
              s => s.signalingPeerId === message.peerId
            );
            ...
            if (scout) {
              ...
              console.log(
                `✅ Stored scout mapping: peerId ${message.peerId} -> scoutId ${scoutId}`
              );
            } else {
          ...
          if (mode === 'lead' && message.type === 'answer') {
            ...
            if (scout) {
              ...
              console.warn(
                '⚠️ Context: Received answer but no pending scout found for peerId:',
                message.peerId
              );
            }
          }
          ...
          if (message.type === 'ice-candidate') {
            ...
            console.log(
              '🧊 Context: Received ICE candidate from',
              message.peerName,
              'peerId:',
              message.peerId,
              'mode:',
            ...
            try {
              ...
              if (mode === 'lead') {
                ...
                console.log(
                  'Connected scouts:',
                  connectedScoutsRef.current.map(s => ({ id: s.id, name: s.name }))
                );
                ...
                if (scout) {
                  console.log(
                    `✅ Scout found: ${scout.name}, connection exists: ${!!scout.connection}`
                  );
                }

                if (scout?.connection) {
                  console.log(
                    `✅ Context: Adding ICE candidate to scout ${scout.name} (signaling state: ${scout.connection.signalingState})`
                  );
                  ...
                  if (scout.connection.remoteDescription) {
                    ...
                    console.warn(
                      '⚠️ Context: Remote description not set yet, cannot add ICE candidate'
                    );
                  }
                } else {
                  console.warn(
                    `⚠️ Context: No scout connection found for peerId ${message.peerId}`
                  );
                  console.warn(`Pending scouts:`, Array.from(pendingScoutsRef.current.keys()));
                  console.warn(
                    `Connected scouts:`,
                    connectedScoutsRef.current.map(s => s.id)
                  );
                }
              } else if (mode === 'scout') {
                ...
                if (scoutConnectionRef.current && scoutConnectionRef.current.remoteDescription) {
                  console.log(
                    `✅ Context: Adding ICE candidate (signaling state: ${scoutConnectionRef.current.signalingState})`
                  );
                  ...
                  pendingIceCandidatesRef.current
                    .get(bufferKey)!
                    .push(message.data as RTCIceCandidateInit);
                  console.log(
                    `📦 Context: ${pendingIceCandidatesRef.current.get(bufferKey)!.length} candidates buffered`
                  );
                }
              }
            } catch (err) {
  ...
  useEffect(() => {
    ...
    if (mode === 'scout' && roomCode && signaling && !isConnected && !hasAutoJoinedRef.current) {
      ...
      if (savedRoomCode === roomCode) {
        console.log(
          '🔄 Context: Auto-joining room',
          roomCode,
          'from any page (current status:',
          connectionStatus,
          ')'
        );
