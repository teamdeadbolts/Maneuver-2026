...
import { exportScoutingData, savePitScoutingEntries } from '@/core/db/database';
...
const QRDataTransferPage = () => {
  ...
  const saveScoutingDataWithConflicts = useCallback(
    async (data: unknown) => {
      const scoutingData = data as {
      ...
      <div className="h-screen w-full flex flex-col items-center px-4 pt-12 pb-24">
        <div className="flex flex-col items-start gap-4 max-w-md w-full">
          ...
          <p className="text-muted-foreground">
            Transfer large data files using fountain codes. Scan packets in any order until
            reconstruction is complete. Note: Pit scouting transfers include text data only - use
            JSON transfer for images.
          </p>
          ...
          <Card className="w-full">
            <CardHeader>
              ...
              <CardDescription>Choose what type of data you want to transfer</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <div className="flex flex-col gap-4 w-full">
            <Button onClick={() => setMode('generate')} className="w-full h-16 text-xl">
