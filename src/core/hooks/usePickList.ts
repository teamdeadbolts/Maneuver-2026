...
export const usePickList = (eventKey?: string): UsePickListResult => {
  ...
  const addTeamToList = useCallback(
    (team: TeamStats, listId: number) => {
      ...
      setPickLists(prev =>
        prev.map(list => (list.id === listId ? { ...list, teams: [...list.teams, newItem] } : list))
      );
  ...
  const updateListTeams = useCallback((listId: number, newTeams: PickListItem[]) => {
    setPickLists(prev =>
      prev.map(list => (list.id === listId ? { ...list, teams: newTeams } : list))
    );
  }, []);
  ...
  // Import pick lists
  const importPickLists = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = e => {
        try {
          ...
          // Validate pick lists
          const validPickLists = importedData.filter(
            (list): list is PickList =>
              typeof list === 'object' &&
              list !== null &&
              'id' in list &&
              'name' in list &&
  ...
  const addTeamToAlliance = useCallback(
    (teamNumber: number, allianceId: number) => {
      ...
      setAlliances(prev =>
        prev.map(a => (a.id === allianceId ? { ...a, [position]: teamNumber } : a))
      );
      ...
      toast.success(
        `Team ${teamNumber} assigned as ${positionNames[position]} of Alliance ${alliance.allianceNumber}`
      );
    },
    [alliances]
  );
  ...
  const assignToAllianceAndRemove = useCallback(
    (teamNumber: number, allianceIndex: number) => {
      ...
      // Update alliance
      setAlliances(prev =>
        prev.map((a, index) => (index === allianceIndex ? { ...a, [position]: teamNumber } : a))
      );
      ...
      toast.success(
        `Team ${teamNumber} added to Alliance ${alliance.allianceNumber} as ${positionNames[position]}`
      );
    },
    [alliances]
  );
