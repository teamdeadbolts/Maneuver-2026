...
export function useMatchValidation({
  ...
  const [validationResults, setValidationResults] = useState<Map<string, MatchValidationResult>>(
    new Map()
  );
    ...
    try {
      ...
      for (const item of items) {
        ...
        const matchEntries = scoutingEntries.filter(
          entry => entry.matchKey === item.matchKey || entry.matchNumber === item.matchNumber
        );

        // Count scouted teams per alliance
        const redScouted = new Set(
          matchEntries.filter(e => e.allianceColor === 'red').map(e => e.teamNumber.toString())
        );
        const blueScouted = new Set(
          matchEntries.filter(e => e.allianceColor === 'blue').map(e => e.teamNumber.toString())
        );
  ...
  const validateSingleMatch = async (
    match: MatchListItem
    ...
    try {
      ...
      const redAlliance = buildAllianceValidation(
        'red',
        redScouted,
        redTBA,
        redDiscrepancies,
      ...
      const blueAlliance = buildAllianceValidation(
        'blue',
        blueScouted,
        blueTBA,
        blueDiscrepancies,
      ...
      const teams = buildTeamValidations(
        redEntries,
        blueEntries,
        redDiscrepancies,
        blueDiscrepancies
      ...
      console.log(
        '[Validation] Created result with status:',
        result.status,
        'for:',
        match.matchKey
      );
  ...
  const validateMatch = useCallback(
    async (matchKey: string): Promise<MatchValidationResult | null> => {
  ...
  const getMatchResult = useCallback(
    (matchKey: string): MatchValidationResult | null => {
 ...
 */
async function getScoutingEntriesForEvent(eventKey: string): Promise<
  Array<{
    matchKey: string;
    matchNumber: number;
    teamNumber: number;
...
function aggregateScoutingData(
  ...
  for (const entry of entries) {
    ...
    const autoPhase =
      gameData.auto && typeof gameData.auto === 'object'
        ? (gameData.auto as Record<string, unknown>)
        : null;
    const teleopPhase =
      gameData.teleop && typeof gameData.teleop === 'object'
    ...
    const autoFuel =
      ...
      (typeof flatGameData.autoFuelScoredCount === 'number'
        ? flatGameData.autoFuelScoredCount
        : undefined) ??
      0;

    const teleopFuel =
      (typeof teleopPhase?.fuelScoredCount === 'number'
        ? teleopPhase.fuelScoredCount
        : undefined) ??
      (typeof teleopPhase?.fuelScored === 'number' ? teleopPhase.fuelScored : undefined) ??
      (typeof flatGameData.teleopFuelScored === 'number'
        ? flatGameData.teleopFuelScored
        : undefined) ??
      (typeof flatGameData.teleopFuelScoredCount === 'number'
        ? flatGameData.teleopFuelScoredCount
        : undefined) ??
      0;
...
function buildAllianceValidation(
  ...
  return {
    ...
    status,
    confidence:
      scouted.scoutedTeamsCount === 3 ? 'high' : scouted.scoutedTeamsCount >= 1 ? 'medium' : 'low',
    discrepancies,
    totalScoutedPoints,
