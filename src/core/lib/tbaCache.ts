...
export async function getCachedTBAEventMatches(
  ...
  const cached = await db.matches.where('eventKey').equals(eventKey).toArray();

  const now = Date.now();

  // Offline-first: Return all data unless explicitly filtering expired
  const validCached = includeExpired ? cached : cached.filter(c => now <= c.expiresAt);
...
export async function clearExpiredCache(): Promise<number> {
  const now = Date.now();
  const expired = await db.matches.where('expiresAt').below(now).toArray();
...
export async function getTBACacheStats(): Promise<{
  ...
  const now = Date.now();
  const expiredMatches = await db.matches.where('expiresAt').below(now).count();
 ...
 */
export async function getEventValidationResults(eventKey: string): Promise<ValidationResultDB[]> {
  return await db.validationResults.where('eventKey').equals(eventKey).toArray();
}
 ...
 * @param eventKey - Event key
 */
export async function clearEventValidationResults(eventKey: string): Promise<void> {
  await db.validationResults.where('eventKey').equals(eventKey).delete();
}
