...
export const handlePitScoutingImagesUpload = async (jsonData: unknown): Promise<void> => {
  ...
  try {
    ...
    if (result.updated === 0 && result.notFound > 0) {
      toast.error(
        `No matching pit scouting entries found for ${result.notFound} teams. Please import pit scouting text data first, then add images.`
      );
    } else if (result.notFound > 0) {
      toast.warning(
        `Updated ${result.updated} teams with images. ${result.notFound} teams not found - ensure pit scouting entries exist first.`
      );
    } else {
