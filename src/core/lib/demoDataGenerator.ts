...
function generateTeamProfiles(): TeamSkillProfile[] {
  ...
  for (let i = 0; i < eliteCount; i++) {
    teams.push({
      ...
      teleopAccuracy: 0.8 + Math.random() * 0.2,
      endgameSuccess: 0.9 + Math.random() * 0.1,
  ...
  for (let i = 0; i < strongCount; i++) {
    teams.push({
      ...
      skillLevel: 'strong',
      tier: 'strong',
      autoAccuracy: 0.65 + Math.random() * 0.2,
      teleopAccuracy: 0.65 + Math.random() * 0.2,
      endgameSuccess: 0.7 + Math.random() * 0.2,
      consistency: 0.7 + Math.random() * 0.2,
    });
  }

  // Average teams (next 40%)
  const averageCount = Math.ceil(TEAMS_COUNT * 0.4);
  for (let i = 0; i < averageCount; i++) {
    teams.push({
      teamNumber: 3000 + i,
      skillLevel: 'average',
      tier: 'average',
      autoAccuracy: 0.4 + Math.random() * 0.3,
      teleopAccuracy: 0.45 + Math.random() * 0.3,
      endgameSuccess: 0.5 + Math.random() * 0.3,
      consistency: 0.5 + Math.random() * 0.3,
    });
  }
  ...
  for (let i = 0; i < remaining; i++) {
    teams.push({
      ...
      tier: 'developing',
      autoAccuracy: 0.2 + Math.random() * 0.3,
      teleopAccuracy: 0.25 + Math.random() * 0.35,
      endgameSuccess: 0.3 + Math.random() * 0.3,
      consistency: 0.3 + Math.random() * 0.4,
    });
  }
  ...
  const calculateAllianceStrength = (teams: number[]) => {
    ...
    for (const teamNum of teams) {
      ...
      if (profile) {
        ...
        const strength =
          ((profile.autoAccuracy + profile.teleopAccuracy + profile.endgameSuccess) / 3) *
          randomFactor;
...
function getOrCreateMatchStats(
...
function recordDemoEntryStats(
  ...
  allianceStats.endgameTowerResults.push(
    applyOfficialTowerVariance(parseEndgameTowerResult(gameData))
  );
}
...
function buildDemoScoreBreakdown(stats: DemoMatchStats): {
  red: DemoAllianceBreakdown;
  blue: DemoAllianceBreakdown;
} {
  return {
...
function createDemoDiscrepancy(
  ...
  const percentDiff = scoutedValue > 0 ? (absDifference / scoutedValue) * 100 : 0;
...
function createFakeValidationResult(
  ...
  const seedAllianceDiscrepancies = (target: Discrepancy[], roll: number) => {
    ...
    if (roll <= 9) {
      ...
      target.push(
        createDemoDiscrepancy(snapshot, 'totalFuelScored', roll % 2 === 0 ? 'warning' : 'minor')
      );
      if (roll === 9) {
  ...
  const criticalDiscrepancies = [...redDiscrepancies, ...blueDiscrepancies].filter(
    d => d.severity === 'critical'
  ).length;
  const warningDiscrepancies = [...redDiscrepancies, ...blueDiscrepancies].filter(
  ...
  const buildAllianceComparison = (discrepancies: Discrepancy[], baseScoutedPoints: number) => {
    ...
    const scorePercentDiff = tbaPoints > 0 ? (Math.abs(scoreDifference) / tbaPoints) * 100 : 0;
...
export async function generateDemoEventScheduleOnly(
  options: Pick<DemoDataOptions, 'eventKey' | 'clearExisting'> = {}
): Promise<DemoDataResult> {
  const { eventKey = DEMO_EVENT_KEY, clearExisting = true } = options;
  ...
  try {
    ...
    for (const match of allMatches) {
      ...
      for (let i = 0; i < match.redTeams.length; i++) {
        ...
        const scoutName =
          TEST_SCOUT_NAMES[Math.floor(Math.random() * TEST_SCOUT_NAMES.length)] || 'Demo Scout';
    ...
    for (const team of teams) {
      ...
      const fuelCapacity =
        robotHeight >= 24
          ? 80 + Math.floor(Math.random() * 41) // 80-120
          : robotHeight >= 22
            ? 50 + Math.floor(Math.random() * 31) // 50-80
    ...
    const scoutAccuracies: Record<string, number> = {
      ...
      'Sarah Chen': 0.7, // Above average
      'Marcus Rodriguez': 0.65, // Average
      'Taylor Wilson': 0.6, // Slightly below average
      'Emma Thompson': 0.55, // Learning
      'Jordan Smith': 0.5, // Still learning
    ...
    for (const match of allMatches) {
      const actualWinner = determineMatchWinner(match.redTeams, match.blueTeams, teamProfilesMap);
    ...
    console.log(
      `  ✓ Generated ${predictionsGenerated} predictions (${correctPredictions} correct, ${Math.round((correctPredictions / predictionsGenerated) * 100)}% accuracy)`
    );
    ...
    for (const scoutName of TEST_SCOUT_NAMES) {
      ...
      if (scoutPredictions.length > 0) {
        ...
        const totalStakesFromPredictions = scoutPredictions.reduce(
