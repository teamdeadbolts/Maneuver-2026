...
export function useMatchValidationWithScaling(options: UseMatchValidationWithScalingOptions) {
  ...
  const getTBAMatchForScaling = useCallback(
   ...
   */
  const validateMatchWithScaling = useCallback(
    async (matchKey: string): Promise<MatchValidationResult | null> => {
      ...
      try {
        ...
        const matchEntries = allEntries.filter(
          e => normalizeMatchKey(e.matchKey) === normalizedMatchKey
        );
        ...
        const redScaling = calculateAllianceScaling('red', redEntries, redBreakdown);

        const blueScaling = calculateAllianceScaling('blue', blueEntries, blueBreakdown);
  ...
  const validateEventWithScaling = useCallback(async () => {
    ...
    try {
      const matchesToScale = coreValidation.matchList.filter(
        m => m.hasScouting && m.hasTBAResults && m.validationResult
      );
