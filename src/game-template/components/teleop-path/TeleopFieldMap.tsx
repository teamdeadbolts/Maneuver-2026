...
function TeleopFieldMapContent() {
  ...
  const { drawingPoints, handleDrawStart, handleDrawMove, handleDrawEnd, resetDrawing } =
    usePathDrawing({
      canvasRef,
      isFieldRotated,
      alliance,
  ...
  const handleElementClick = (elementKey: string) => {
    ...
    if (elementKey.includes('trench') || elementKey.includes('bump')) {
      ...
      const obstacleZone: ZoneType = elementKey.includes('opponent')
        ? 'opponentZone'
        : 'allianceZone';
  ...
  const content = (
    <div className={cn('flex flex-col gap-2', isFullscreen && 'h-full')}>
      ...
      <FieldHeader
        ...
        actionLogSlot={
          <TeleopActionLog actions={actions} open={actionLogOpen} onOpenChange={setActionLogOpen} />
        }
          ...
          {activeZone && !pendingWaypoint && !isSelectingScore && !isSelectingPass && (
            <>
              {visibleElements.map(key => {
                ...
                if (
                  key === 'defense_alliance' ||
                  key === 'defense_neutral' ||
                  key === 'defense_opponent'
                ) {
                  count = totalDefense;
                } else if (key === 'steal') {
                  count = totalSteal;
          ...
          {!pendingShotTypeWaypoint && (isSelectingScore || isSelectingPass) && (
            ...
            >
              ...
              >
                {isSelectingScore
                  ? disablePathDrawingTapOnly
                    ? 'Tap where robot scored'
                    : 'Tap or draw to shoot'
                  : disablePathDrawingTapOnly
                    ? 'Tap where robot passed'
                    : 'Tap or draw pass path'}
              </div>
              <Button
                variant="ghost"
                size="sm"
            ...
            <PendingWaypointPopup
              ...
              onConfirm={
                pendingWaypoint.type === 'climb'
                  ? selectedClimbStartTimeSecRemaining => {
