...
function getStatColorClass(color: FieldHeaderStat['color']): string {
  switch (color) {
    case 'green':
      return 'bg-green-100 text-green-700 border-green-300 dark:bg-green-600/20 dark:text-green-400 dark:border-green-500/30';
    case 'purple':
      return 'bg-purple-100 text-purple-700 border-purple-300 dark:bg-purple-600/20 dark:text-purple-400 dark:border-purple-500/30';
    case 'yellow':
      return 'bg-yellow-100 text-yellow-800 border-yellow-300 dark:bg-yellow-600/20 dark:text-yellow-400 dark:border-yellow-500/30';
    case 'slate':
      return 'bg-slate-100 text-slate-700 border-slate-300 dark:bg-slate-800/50 dark:text-slate-300 dark:border-slate-700/50';
  }
}
  ...
  return (
    <div className="flex items-center justify-between px-1">
      <div className="flex items-center gap-2 flex-1 min-w-0">
        ...
        {headerInputSlot && <div className="shrink-0">{headerInputSlot}</div>}
          ...
          <div className="flex items-center gap-1 sm:gap-2 ml-auto">
            {stats.map((stat, i) => (
              <Badge
                key={i}
                variant="secondary"
                className={cn('text-[10px] md:text-xs px-1.5 py-0', getStatColorClass(stat.color))}
              >
      ...
      <div className="flex items-center gap-1 pl-4">
        ...
        {onBrokenDownToggle && (
          <Button
            ...
            title={
              isBrokenDown ? 'Robot is broken down - tap to resume' : 'Mark robot as broken down'
            }
          >
        ...
        {onUndo && (
          <Button
            ...
            disabled={!canUndo}
            className={cn(
              'h-8 w-8 hover:bg-slate-200 dark:hover:bg-slate-800 lg:hidden',
              canUndo && 'text-red-500 dark:text-red-400 animate-in fade-in zoom-in duration-300'
            )}
          >
        ...
        <div className="hidden lg:flex lg:items-center lg:gap-1">
          ...
          {onUndo && (
            <Button
              ...
              disabled={!canUndo}
              className={cn(
                'h-8 w-8 hover:bg-slate-200 dark:hover:bg-slate-800',
                canUndo && 'text-red-500 dark:text-red-400 animate-in fade-in zoom-in duration-300'
              )}
            >
              <Undo2 className="h-4 w-4" />
          ...
          {toggleFieldOrientation && (
            <Button
              ...
              className={cn(
                'h-8 w-8 hover:bg-slate-200 dark:hover:bg-slate-800',
                isFieldRotated && 'text-blue-500 dark:text-blue-400'
              )}
              title={isFieldRotated ? 'Reset field orientation' : 'Rotate field 180°'}
            >
              <RotateCw
                className={cn('h-4 w-4 transition-transform', isFieldRotated && 'rotate-180')}
              />
            </Button>
          )}
      ...
      <Dialog open={showNoShowDialog} onOpenChange={setShowNoShowDialog}>
        <DialogContent className="z-150">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure team {teamNumber} did not show up for match{' '}
              {formatMatchLabel(matchNumber || '', matchType)}?
              <br />
              <br />
          ...
          <DialogFooter>
            <Button variant="outline" className="p-4" onClick={() => setShowNoShowDialog(false)}>
              Cancel
            </Button>
            <Button
              variant="destructive"
