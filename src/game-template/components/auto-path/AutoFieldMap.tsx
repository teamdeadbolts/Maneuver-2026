...
import { GAME_SCOUT_OPTION_KEYS, getEffectiveScoutOptions } from '@/game-template/scout-options';
...
function AutoFieldMapContent({
  ...
  const canvasRef = useMemo(
    () => ({
      get current() {
        return fieldCanvasRef.current?.canvas ?? null;
      },
    }),
    []
  ...
  useEffect(() => {
    ...
    if (actions.length === 0) {
      ...
      toast.info(
        `Starting position set to ${START_KEY_LABELS[preferredStartKey]}. Choose the next auto action.`
      );
    }
  ...
  const handleInteractionEnd = (points: { x: number; y: number }[]) => {
    ...
    if (isSelectingScore) {
      const inferredShotType = shouldUsePath
        ? getPathLength(points) >= MOVING_SHOT_MIN_PATH_LENGTH
          ? 'onTheMove'
          : 'stationary'
        : 'stationary';
    ...
    <div className={cn('flex flex-col gap-2', isFullscreen && 'h-full')}>
      ...
      >
        ...
        {isSelectingScore && !pendingShotTypeWaypoint && (
          <div
            className={cn(
              'absolute inset-0 z-30 flex items-end justify-center pb-4 pointer-events-none',
            ...
            <Card className="pointer-events-auto bg-background/95 backdrop-blur-sm border-green-500/50 shadow-2xl py-2 px-3 flex flex-row items-center gap-4">
              ...
              <span className="text-sm font-medium">
                {disablePathDrawingTapOnly
                  ? 'Tap where the robot scored'
                  : 'Tap or draw where the robot scored'}
              </span>
              <Button
                onClick={e => {
                  e.stopPropagation();
        ...
        {isSelectingPass && !pendingShotTypeWaypoint && (
          ...
          >
            <Card className="pointer-events-auto bg-background/95 backdrop-blur-sm border-purple-500/50 shadow-2xl py-2 px-3 flex flex-row items-center gap-4">
              ...
              <span className="text-sm font-medium">
                {disablePathDrawingTapOnly
                  ? 'Tap where the robot passed from'
                  : 'Tap or draw where the robot passed from'}
              </span>
              <Button
                onClick={e => {
                  e.stopPropagation();
        ...
        {isSelectingCollect && !pendingShotTypeWaypoint && (
          ...
          >
            <Card className="pointer-events-auto bg-background/95 backdrop-blur-sm border-yellow-500/50 shadow-2xl py-2 px-3 flex flex-row items-center gap-4">
              ...
              <span className="text-sm font-medium">
                {disablePathDrawingTapOnly
                  ? 'Tap where the robot collected'
                  : 'Tap or draw where the robot collected'}
              </span>
              <Button
                onClick={e => {
                  e.stopPropagation();
          ...
          <PendingWaypointPopup
            ...
            onConfirm={selectedClimbStartTimeSecRemaining => {
              ...
              if (pendingWaypoint.type === 'climb') {
                ...
                delta =
                  pendingWaypoint.type === 'score' || pendingWaypoint.type === 'pass'
                    ? -accumulatedFuel
                    : 0;
                label =
                  pendingWaypoint.type === 'score'
                    ? `+${accumulatedFuel}`
                    : `Pass (${accumulatedFuel})`;
              }
  ...
  if (isFullscreen) {
    return <div className="fixed inset-0 z-100 bg-background p-4 flex flex-col">{content}</div>;
  }
